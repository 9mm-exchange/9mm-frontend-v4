  ...
  const handleRedirectSwap = () => {
    ...
    if (network && tokenAddress) {
      ...
      const url = isNativeToken
        ? `https://dex.9mm.pro/swap?chain=${CHAIN_QUERY_NAME[network]}&persistChain=${network}&outputCurrency=${nativeToken.symbol}`
        : `https://dex.9mm.pro/swap?chain=${CHAIN_QUERY_NAME[network]}&persistChain=${network}&outputCurrency=${tokenAddress}`
      window.open(url, '_blank', 'noopener noreferrer')
    }
  }
