...
const formatDayData = (dayData: DayData): ChartDayData => ({
  ...
  tvlUSD: (dayData.tvlUSD || dayData.totalValueLockedUSD) ?? '0',
})
...
function formatDaysData(data: DayData[], startTimestamp: number, endTimestamp: number): DayData[] {
  return data
    .filter((dayData) => {
      const hasValue = dayData.totalValueLockedUSD || dayData.tvlUSD
      const inRange = dayData.date >= startTimestamp && dayData.date <= endTimestamp
      return hasValue && inRange
    })
    .map((dayData) => ({
      date: dayData.date,
      totalValueLockedUSD: dayData.totalValueLockedUSD ?? dayData.tvlUSD ?? '0',
      ...(dayData.tvlUSD && { tvlUSD: dayData.tvlUSD }),
    }))
