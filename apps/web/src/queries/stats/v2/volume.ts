...
function formatDaysData(data: DayData[], startTimestamp: number, endTimestamp: number): VolumeChartEntry[] {
  return data
    .filter((dayData) => {
      const volume = dayData.dailyVolumeUSD
      return volume && dayData.date >= startTimestamp && dayData.date <= endTimestamp
    })
    .map((dayData) => ({
      date: dayData.date,
      volumeUSD: parseFloat(dayData.dailyVolumeUSD || '0'),
    }))
...
export async function fetchChartData(client: GraphQLClient): Promise<ChartDayData[]> {
  try {
    ...
    return formatDaysData(data, MIN_TIMESTAMP, dayjs.utc().unix()).map(formatDayData)
  } catch (error) {
...
export const fetchTokenVolumeData = (
  address: string,
  client: GraphQLClient,
  period: string = '1Y',
): Promise<ChartDayData[]> => fetchEntityVolumeData(address, client, period, QUERIES.TOKEN)

export const fetchPoolVolumeData = (
  address: string,
  client: GraphQLClient,
  period: string = '1Y',
): Promise<ChartDayData[]> => fetchEntityVolumeData(address, client, period, QUERIES.POOL)
