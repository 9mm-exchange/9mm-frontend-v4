...
import { getTicksData } from 'queries/pools/ticks'
...
const handler: NextApiHandler = async (req, res) => {
  try {
    const { chainName, address, after } = req.query
    ...
    const tickData = await getTicksData(address, chainId, after as string)

    if (tickData.error || !tickData.rows.length) {
