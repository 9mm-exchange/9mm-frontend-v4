...
import { fetchPoolVolumeChartData } from 'queries/stats/volume'
...
/**
 * Redis-backed Volume data fetcher with fallback caching strategy:
...
const getPoolVolumeChartDataWithRedis = async (
  ...
  const cacheKey = `pool-volume:${chainId}:${address.toLowerCase()}:${period || 'default'}`
  ...
  try {
    ...
    return {
      data: null,
      error: 'Failed to fetch volume data with cache fallback',
      isCached: false, // Flag to indicate if we're serving stale data
    }
  }
}
 ...
 */
const cachedFetchPoolVolumeChartData = unstableCache(
  getPoolVolumeChartDataWithRedis,
  ['pool-volume-chart'], // Cache key prefix for memory cache
  {
    revalidate: CACHE_DURATION,
    tags: ['pool-volume'], // For manual revalidation via On-Demand ISR
  },
)
  ...
  try {
    ...
    if (!chainName || !address) {
      return res.status(400).json({
        ...
        documentation: 'https://docs.your-api.com/endpoints/volume#required-parameters',
      })
    }
    ...
    if (!chainId) {
      return res.status(400).json({
        ...
        documentation: 'https://docs.your-api.com/endpoints/volume#supported-chains',
      })
    }
    ...
    if (typeof address !== 'string' || !address.match(/^0x[a-fA-F0-9]{40}$/)) {
      return res.status(400).json({
        ...
        documentation: 'https://docs.your-api.com/endpoints/volume#address-format',
      })
    }
    ...
    if (period && !validPeriods.includes(period as string)) {
      return res.status(400).json({
        ...
        documentation: 'https://docs.your-api.com/endpoints/volume#time-periods',
      })
    }
    ...
    if (volumeData.error || !volumeData.data) {
      ...
      return res.status(404).json({
        ...
        details: volumeData.error || 'No data available for this pool',
        documentation: 'https://docs.your-api.com/endpoints/volume#troubleshooting',
      })
    }
    ...
    return res.status(500).json({
      ...
      documentation: 'https://docs.your-api.com/endpoints/volume#server-errors',
