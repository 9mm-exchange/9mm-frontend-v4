...
const handler: NextApiHandler = async (req, res) => {
  try {
    ...
    const transactionData = await fetchTopTransactions(chainId, token?.toLowerCase())
    if (transactionData.error || !transactionData.data) {
      return res.status(404).json({ name: 'Error', message: 'no result' })
    }

    // Return successful response
    res.setHeader('Cache-Control', CACHE_HEADERS['Cache-Control'])
    return res.status(200).json(transactionData.data)
  } catch (error) {
